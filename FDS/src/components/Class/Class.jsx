...
function Class(props) {
    ...
    if (props.coordinator != '1') {
        if (props.group === '1') {
            ...
            link = '/studentDashboard/' + props.courseID
        } else {
            //Verify levels array length and extract the data 
            if (props.levels.length == 0) {
                var level = "N/A"
    ...
    if (props.coordinator === '1' || props.group === '0') {
        ...
        return (
            <div className='class-box'>
                <div className={`class-head ${getRandomClassID()}`}>
                    ...
                    {/* <h2>{props.period.name}</h2> */}
                    {props.coordinator === '1' ? <h2>{props.teacher.first_name} {props.teacher.last_name}</h2> : <h2></h2>}
                </div>
                <div className='class-body'>
                    {/* ... */}
                </div>
